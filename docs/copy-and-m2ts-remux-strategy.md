...
1. 主 Play-All 中每个正片 PlayItem 都是时长相近、完整覆盖不同源 M2TS 的一集，起点具有 Entry Mark 或 `connection_condition=1` 的非无缝硬边界；允许每端至多一个、两端累计不超过 30 秒且同样独立并带硬边界的版权卡/标识片段，计划会记录被排除时长；
