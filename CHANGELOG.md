...
- 加入批量 `--season`，季号优先级为逐盘 TOML、CLI、原盘明确季标、默认 Season 1；无证据的默认值不阻止构建，但以低置信度和结构化待复核字段贯穿 plan、results 与 state。
- 作品目录名按逐盘配置、CLI、原盘 META、原盘目录名回退，并在 plan、results 与 state 中记录最终标题来源；任务级不设置全局标题，避免多作品批次被错误归组。
- 执行器会把作品标题、季号和集号审计字段重新绑定到实际输出路径，并拒绝未知来源值；超长剧集标题截断时保留 Emby 的 `SxxExx` 标识。
...
- 完整、唯一且时长一致的单集 PlayItem 现在也可由 `connection_condition=1` 的非无缝硬边界证明；主 Play-All 每端至多一个、两端累计不超过 30 秒且同样独立的版权卡或片头标识会带警告排除，不再把整盘误判成单集，执行器会按同一规则重新证明计划。
